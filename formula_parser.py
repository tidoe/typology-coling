	...
	def __init__(self, operands, operator=None):
		...
		if self.operator is None:
			if len(self.operands) > 1:
				...
				raise ValueError("Atomic terms must have exactly one string operand")
	...
	while len(formula) > 1:
		...
		if i2-i1 == 1:
			if type(formula[i1]) == str:
				formula = formula[:i1-b] + [Term(formula[i1:i2])] + formula[i2+b:]
			else:
			...
			if len(operators) > 1:
				raise ValueError("Invalid formula")
			formula = formula[:i1-b] + [Term(operands, list(operators)[0])] + formula[i2+b:]
	return formula[0]
...
def traverse_preorder(term):
	...
	order = term.operator + " ( " + " ".join([traverse_preorder(t) for t in term.operands]) + " ) "
...
def traverse_inorder(term):
	...
	if len(term.operands) == 1:
		order = " ( " + term.operator + " " + traverse_inorder(term.operands[0]) + " ) "
	else:
		order = " ( " + (" " + term.operator + " ").join([traverse_inorder(t) for t in term.operands]) + " ) "
	return " ".join(order.strip().split())
...
def traverse_postorder(term):
	...
	if term.operator is None:
		return term.operands[0]
	order = " ( " + " ".join([traverse_postorder(t) for t in term.operands]) + " ) " + term.operator
	return " ".join(order.strip().split())
...
def convert_to_dnf(term):
	...
	for conjunct in conjuncts:
		dnf = dnf.replace(conjunct, "( " + conjunct[4:-1].replace(", ", " & ") + " )")
	if dnf.startswith("Or("):
		dnf = dnf[3:-1].replace(", ", " | ")
	dnf = " ".join([("( " + x.replace("~", "¬ ") + " )" if "~" in x else x) for x in dnf.split()])
	dnf = " ".join([("( " + x.replace("___ADD___", " + ") + " )" if "___ADD___" in x else x) for x in dnf.split()])
...
if __name__ == "__main__":
	...
	print(formula)
	print(traverse_preorder(term))
	print(traverse_inorder(term))
	print(traverse_postorder(term))
